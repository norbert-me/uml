...
function erase(paths, erasePath, eraseRadius, allpathspecs) {
    ...
    original_drawing_paths = paths;

    linospecs = allpathspecs;
    ...
    */
    ...
    var pointErase = function (path, whichindex) {
        ...
        while (i < (path.length - 1)) {
            ...
            //   was the first point in the path.
                ...
                var x = getCircleIntersection(p0[0], p0[1], p1[0], p1[1], eX, eY, eraseRadius, whichindex);
                if (x) {
                    path[i] = x;
                    last = i;
                }
            ...
            //   contribute to a new path.  Processing then continues at p1.
                ...
                var x = getCircleIntersection(p1[0], p1[1], p0[0], p0[1], eX, eY, eraseRadius, whichindex);
                if (x) {
                ...
                var possIntersects = getCircleIntersections(p0[0], p0[1], p1[0], p1[1], eX, eY, eraseRadius, whichindex);
                if (possIntersects) {
    ...
    var capsuleErase = function (path, i, whichindex) {
        var e0 = erasePath[i];
        ...
        while (i < (path.length - 1)) {
            ...
            //   was the first point in the path.
                ...
                var x = getCapsuleIntersection(p0[0], p0[1], p0_locationIndex, p1[0], p1[1], e0[0], e0[1], e1[0], e1[1], eraseRadius, whichindex);
                if (x) {
                    path[i] = x;
                    last = i;
                }
            ...
            //   contribute to a new path.  Processing then continues at p1.
                ...
                var x = getCapsuleIntersection(p1[0], p1[1], p1_locationIndex, p0[0], p0[1], e0[0], e0[1], e1[0], e1[1], eraseRadius, whichindex);
                if (x) {
                    newPath = path.slice(last, i + 1);
                ...
                // Neither p0 or p1 is in the erase area, so there may or may not be a pair of intersections.
                var possIntersects = getCapsuleIntersections(p0[0], p0[1], p1[0], p1[1], e0[0], e0[1], e1[0], e1[1], eraseRadius, whichindex);
                if (possIntersects) {
                    // create a new path that goes from the beginning of our current path
                    // to the intersection point
                    newPath = path.slice(last, i + 1);
...
*/
function getCircleIntersection(aX, aY, bX, bY, cX, cY, r, iozeb) {
...
*/
function getCircleIntersections(aX, aY, bX, bY, cX, cY, r, iozeb) {
    var vec_ac = [(cX - aX), (cY - aY)];
    var vec_ab = [(bX - aX), (bY - aY)];
    ...
    var dist = getLengthErase([closest[0] - cX, closest[1] - cY]);
...
*/
function getCapsuleIntersection(aX, aY, locationIndex, bX, bY, c0_x, c0_y, c1_x, c1_y, r, iozeb) {
    var box = getParallelSegments(c0_x, c0_y, c1_x, c1_y, r);
    var intersections = [];

    if (locationIndex[0] && !locationIndex[1]) {
        intersections.push(getCircleIntersection(aX, aY, bX, bY, c0_x, c0_y, r, iozeb));
        ...
        var i = getCircleIntersections(aX, aY, bX, bY, c1_x, c1_y, r, iozeb);
        if (i) intersections.push(i[0], i[1]);
    }
    else if (!locationIndex[0] && locationIndex[1]) {
        intersections.push(getCircleIntersection(aX, aY, bX, bY, c1_x, c1_y, r, iozeb));
        intersections.push(getLineIntersection(aX, aY, bX, bY, box[0][0], box[0][1], box[3][0], box[3][1]));
        intersections.push(getLineIntersection(aX, aY, bX, bY, box[1][0], box[1][1], box[2][0], box[2][1]));
        var i = getCircleIntersections(aX, aY, bX, bY, c0_x, c0_y, r, iozeb);
        if (i) intersections.push(i[0], i[1]);
    }
    else if (locationIndex[0] && locationIndex[1]) {
        intersections.push(getCircleIntersection(aX, aY, bX, bY, c0_x, c0_y, r, iozeb));
        ...
        var i = getCircleIntersections(aX, aY, bX, bY, c1_x, c1_y, r, iozeb);
        var j = getCircleIntersections(aX, aY, bX, bY, c0_x, c0_y, r, iozeb);
        if (i) intersections.push(i[0], i[1]);
...
*/
function getCapsuleIntersections(aX, aY, bX, bY, c0_x, c0_y, c1_x, c1_y, r, iozeb) {
    var box = getParallelSegments(c0_x, c0_y, c1_x, c1_y, r);
    var intersections = [];

    var tmp = getCircleIntersections(aX, aY, bX, bY, c0_x, c0_y, r, iozeb);
    if (tmp) intersections.push(tmp[0], tmp[1]);
    tmp = getCircleIntersections(aX, aY, bX, bY, c1_x, c1_y, r, iozeb);
    if (tmp) intersections.push(tmp[0], tmp[1]);
    intersections.push(getLineIntersection(aX, aY, bX, bY, box[0][0], box[0][1], box[3][0], box[3][1]));
    intersections.push(getLineIntersection(aX, aY, bX, bY, box[1][0], box[1][1], box[2][0], box[2][1]));
